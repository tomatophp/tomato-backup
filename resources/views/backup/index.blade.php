<x-tomato-admin-layout>
    ...
    <x-slot name="headerBody">
        <Link slideover href="/admin/backup/create" class="filament-button inline-flex items-center justify-center py-1 gap-1 font-medium rounded-lg border transition-colors focus:outline-none focus:ring-offset-2 focus:ring-2 focus:ring-inset dark:focus:ring-offset-0 min-h-[2.25rem] px-4 text-sm text-white shadow focus:ring-white border-transparent bg-primary-600 hover:bg-primary-500 focus:bg-primary-700 focus:ring-offset-primary-700 filament-page-button-action">
            ...
            <x-splade-table :for="$table" striped>
                <x-splade-cell actions>
                    <div class="flex justify-start">
                        <a href="/admin/backup/{{ $item->id }}"  class="px-2 text-blue-500" target="_blank">
                            <div class="flex justify-start space-x-2">
                                <x-heroicon-s-cloud class="h-4 w-4"/>
                                <span>{{trans('tomato-backup::global.download')}}</span>
                            </div>
                        </a>
                        <Link href="/admin/backup/{{ $item->id }}"
